...
#region Variables
	...
	void Start()
	{
		...
		BlockObj = GameObject.FindGameObjectsWithTag("Block");
	}
	...
	public void Help()
	{
		if (BlockObj.Length > 0 && !isHelp)
		{
			for (int i = 0; i < BlockObj.Length; i++)
			{
				float oriX = BlockObj[i].transform.localScale.x;
				float oriY = BlockObj[i].transform.localScale.y;
				BlockObj[i].transform.localScale = new Vector3(oriX * 0.75f, oriY * 0.75f, 1);
			}
			isHelp = true;
		}
	}
	...
	public void SetState(States NextState)
	{
		...
		if (State == States.WIN)
		{
			...
			if(GameObject.Find("AdBtn")!=null)
            {
				GameObject.Find("AdBtn").SetActive(false);
			}
